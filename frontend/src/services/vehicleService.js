...
export const getVehiclesAPI = async (filters = {}) => {
  ...
  if (!response.ok) throw new Error(data.message || "Không thể tải danh sách phương tiện.");
  return data;
};
...
export const getDashboardStatsAPI = async () => {
  ...
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || "Không thể tải số liệu thống kê.");
  return data;
};
...
export const registerVehicleAPI = async (vehicleData) => {
  ...
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || "Đăng ký phương tiện thất bại.");
  return data;
};

// 3. Gia hạn vé
export const extendVehicleAPI = async (id, months) => {
...
export const updateVehicleAPI = async (id, vehicleData) => {
  ...
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || "Cập nhật phương tiện thất bại.");
  return data;
};
...
export const getRevenueReportAPI = async () => {
  ...
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || "Không thể tải báo cáo doanh thu.");
  return data;
};

// 8. Lấy nhật ký xe ra vào bãi
export const getParkingLogsAPI = async (search = "") => {
  const response = await fetch(`${API_URL}/reports/logs?search=${encodeURIComponent(search)}`, {
    method: "GET",
    headers: getAuthHeaders(),
  });

  const data = await response.json();
  if (!response.ok) throw new Error(data.message || "Không thể tải nhật ký ra vào.");
  return data;
};
...
export const getAlertLogsAPI = async () => {
  ...
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || "Không thể tải nhật ký cảnh báo.");
  return data;
};
